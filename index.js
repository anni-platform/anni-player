...
function useCanvasScrubber({
  fps = DEFAULT_FPS,
  frames = []
}) {
  const sortedFrames = useMemo(() => frames.sort(sortFramesAscending), [frames]);
  ...
  function drawFrame(img) {
    ...
    canvasRef.current.getContext('2d').drawImage(img, 0, 0, img.width, img.height, 0, 0, img.width, img.height);
  }
  ...
  useEffect(() => {
    ...
    async function load() {
      const htmlImages = await Promise.all(sortedFrames.map(f => preloadImagePromise(f)));
    ...
    if (isPlaying) {
      ...
      const nextTick = () => {
        ...
        if (delta > interval) {
          ...
          then = now;
          currentFrame.current = frameIndex === frames.length - 1 ? 0 : frameIndex + 1; // Draw Canvas
...
export default useCanvasScrubber;
